...
const pagesContentService = {
  ...
  createContent: async (formData) => {
    const response = await api.post(ADMIN_PAGES_CONTENT_ENDPOINTS.CREATE_CONTENT, formData, {
      headers: {
  ...
  deleteContent: async (id) => {
    const response = await api.delete(ADMIN_PAGES_CONTENT_ENDPOINTS.DELETE_CONTENT(id));
    return response.data;
  },
};
